...
Merged from backend/definitions/tools.py — all 12 tools (8 ES|QL + 4 custom).
...
ESQL_TOOLS: list[dict] = [
    ...
    },
    {
        ...
        "type": "esql",
        "description": "Find hosts with disk usage above 90%.",
        "configuration": {
            "esqlQuery": (
                "FROM metrics-* "
                "| WHERE @timestamp >= NOW() - 15 MINUTES "
                "| WHERE system.filesystem.used.pct IS NOT NULL "
                "| STATS max_disk = MAX(system.filesystem.used.pct) "
    ...
    },
    {
        ...
        "type": "esql",
        "description": (
            "Detect network connectivity issues — connection timeouts, "
...
# ── Custom Tools (4) — for remediation actions ─────────────────────────────
...
"""All 12 tool definitions (8 ES|QL + 4 custom)."""
